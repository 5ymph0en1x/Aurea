...
pub fn codon_structural_map(
    ...
    use crate::calibration::{Q_A, Q_ALPHA, Q_BETA, Q_GAMMA, CODON_STRUCT_FACTOR_MIN, CODON_STRUCT_FACTOR_MAX};
    ...
    let f_min = CODON_STRUCT_FACTOR_MIN; // 0.80 (preserve structure)
    let f_max = CODON_STRUCT_FACTOR_MAX; // 1.30 (compress smooth gas)
...
/// QMAT with rotated anisotropy axis. Same formula as `qmat_for_block_size` but
/// the (r, c) coordinates are rotated by `angle` radians before evaluation.
/// This aligns the quantization ellipse with the local gradient direction.
pub fn qmat_rotated_for_block_size(row: usize, col: usize, block_size: usize, angle: f64) -> f64 {
    if row == 0 && col == 0 {
        return 0.0;
    }
    let max_index = (block_size - 1) as f64;
    ...
    let rr = r * cos_a + c * sin_a;
    let cc = -r * sin_a + c * cos_a;
    let d = (rr * rr + Q_ALPHA * cc * cc).sqrt();
    let q = Q_A * d.powf(Q_BETA) + Q_GAMMA * rr * cc;
    q.max(0.1)
}

/// Precompute a full rotated QMAT for a given block size and angle, raised to `power`.
/// Returns a flat Vec of size block_size², indexed as [row * block_size + col].
pub fn precompute_rotated_qmat(block_size: usize, angle: f64, power: f64) -> Vec<f64> {
    let n = block_size * block_size;
    let mut qmat = vec![0.0f64; n];
    for row in 0..block_size {
        for col in 0..block_size {
            let q = qmat_rotated_for_block_size(row, col, block_size, angle);
            qmat[row * block_size + col] = q.powf(power);
        }
    }
    qmat
}
...
// ---------------------------------------------------------------------------
// Precomputed QMAT tables — single source of truth: qmat_for_block_size()
// ---------------------------------------------------------------------------
// Each table is computed once on first access via LazyLock, turning every
// subsequent lookup into a flat array index instead of a powf() call.
// All three sizes from BLOCK_SIZES (8, 16, 32) are covered.

/// Helper: build a QMAT lookup table for a given block size.
fn build_qmat<const N: usize>(block_size: usize) -> [f64; N] {
    let mut table = [0.0; N];
    for row in 0..block_size {
        for col in 0..block_size {
            table[row * block_size + col] = qmat_for_block_size(row, col, block_size);
        }
    }
    table
}

pub static QMAT_8: LazyLock<[f64; 64]> = LazyLock::new(|| build_qmat::<64>(8));
pub static QMAT_16: LazyLock<[f64; 256]> = LazyLock::new(|| build_qmat::<256>(16));
pub static QMAT_32: LazyLock<[f64; 1024]> = LazyLock::new(|| build_qmat::<1024>(32));

/// Fast QMAT lookup for any supported block size (8, 16, 32).
/// Falls back to qmat_for_block_size() for unsupported sizes.
#[inline]
pub fn qmat_lookup(row: usize, col: usize, block_size: usize) -> f64 {
    let idx = row * block_size + col;
    match block_size {
        8  => QMAT_8[idx],
        16 => QMAT_16[idx],
        32 => QMAT_32[idx],
        _  => qmat_for_block_size(row, col, block_size),
    }
}
