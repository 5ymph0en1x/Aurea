...
pub struct TuringField {
    ...
    /// Gradient direction of the inhibitor field: atan2(gy, gx) in radians. Size: grid_h × grid_w.
    pub inhibitor_gradient_angle: Vec<f64>,
    pub grid_h: usize,
    pub grid_w: usize,
}
...
/// Sobel 3×3 gradient direction: atan2(gy, gx) in radians per cell.
/// Same kernel as sobel_magnitude but returns the angle instead of the magnitude.
pub fn sobel_direction(grid: &[f64], h: usize, w: usize) -> Vec<f64> {
    assert_eq!(grid.len(), h * w, "sobel_direction: grid size mismatch");
    let mut out = vec![0.0f64; h * w];
...
pub fn compute_turing_field(
    ...
    // Step 7: Inhibitor gradient (magnitude + direction)
    let inhibitor_gradient = sobel_magnitude(&inhibitor, grid_h, grid_w);
    let inhibitor_gradient_angle = sobel_direction(&inhibitor, grid_h, grid_w);

    TuringField {
        field: t_norm,
        step_modulation,
        inhibitor_gradient,
        inhibitor_gradient_angle,
        grid_h,
        grid_w,
    }
}
